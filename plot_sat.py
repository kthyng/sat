...
for row in soup.findAll('a')[5:]:  # loop through each day
    ...
    for File in soup_dir.findAll('a')[5:]:  # find all files for this day
        ...
        if fname in File.string:
            ...
            foo = np.asarray(img)
            # if not date == datetime(2017, 1, 8, 17, 5):
            #     continue
            ...
            if plottide:
                ...
                # startdatestr = (startdatetime).isoformat()[0:10]
                # enddatestr = (startdatetime + timedelta(days=2.5)).isoformat()[0:10]
                ...
                df['east'] = dfw[' WINDSPEED']*np.cos(np.deg2rad(theta))
                df['north'] = dfw[' WINDSPEED']*np.sin(np.deg2rad(theta))
                df.idx = date2num(df.index.to_pydatetime())  # in units of days


                axwind = fig.add_axes([0.51, 0.83, 0.24, 0.08])
                ddt = 1
                axwind.quiver(df.idx[::ddt], np.zeros(len(df[::ddt])), df[::ddt]['east'], df[::ddt]['north'], headaxislength=0,
